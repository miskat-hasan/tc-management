...
const INSTRUCTOR_ROLE_ID = 3; // fixed
...
const AddInstructor = () => {
  ...
  const onSubmit = (values) => {
    // Prevent duplicate training sites
  ...
  // Get already selected site ids to warn on duplicates
  const watchedSites = form.watch("trainingSites");
  const selectedSiteIds = watchedSites.map((ts) => ts.tsite_id).filter(Boolean);
      ...
      <div className="p-[26px] bg-white rounded-[14px] flex flex-col gap-[24px]">
        <FormContainer form={form} onSubmit={onSubmit}>
          <div className="grid grid-cols-2 gap-6">
            ...
            {/* Training Sites */}
            <div className="col-span-1 bg-neutral-50 border px-2 pt-2 pb-4 rounded-md">
              <h6 className="text-lg mb-1">Training Sites</h6>
                  ...
                  <div key={field.id} className="mt-3 border-b pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      {fields.length > 1 && (
                        <div
                          onClick={() => remove(index)}
                          className="bg-neutral-200 p-2 rounded-md cursor-pointer hover:bg-neutral-300 shrink-0"
                        >
                          <LucideTrash2 className="size-4" />
                        </div>
                      )}
